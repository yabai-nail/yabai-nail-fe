...
export function designDeleteErrorDetails(error: ApiClientError): string {
...
/**
 * Confirms a hard delete. Unlike a service, a design has no booking history the API might
 * refuse to lose, so the only thing to warn about is what the customer side loses: the design
 * leaves the app and every customer's favourites at once. Hiding is offered as the gentler
 * alternative in the hint.
 */
export function DesignDeleteModal({
  design,
  onClose,
  onDeleted,
}: Readonly<{
  ...
  const remove = async () => {
    ...
    setError(null);
    try {
      await adminService.deleteNailDesign(design.id, design.version);
      notifySuccess(t("delete.success", { name: design.title }));
      onDeleted();
      onClose();
    } catch (thrown) {
      setError(
        thrown instanceof ApiClientError
          ? t("delete.failedWithDetails", { details: designDeleteErrorDetails(thrown) })
          : t("delete.failed"),
      );
    } finally {
      setBusy(false);
          ...
          <Modal.Dialog>
            <Modal.Header className="flex flex-row items-center gap-3 border-b border-admin-border px-5 py-4">
              ...
              <div>
                ...
                <p className="mt-0.5 text-xs text-admin-muted">{design.title}</p>
              </div>
            </Modal.Header>
            <Modal.Body className="space-y-3 px-5 py-5 text-sm">
              <p className="leading-6 text-admin-ink">{t("delete.confirm", { name: design.title })}</p>
