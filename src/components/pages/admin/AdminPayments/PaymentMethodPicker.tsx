...
const methods = [
  ...
  { id: "paypay", icon: DevicePhoneMobileIcon },
  { id: "visa", icon: CreditCardIcon },
] as const;
...
export function PaymentMethodPicker({ value, isDisabled, onChange }: Readonly<{
  ...
  return (
    <RadioGroup aria-label={t("methodPicker.label")} value={value} onChange={(next) => onChange(next as PaymentMethod)} isDisabled={isDisabled} className="grid gap-2 sm:grid-cols-3">
      ...
      <p className="text-xs leading-5 text-admin-muted sm:col-span-3">{t("methodPicker.counterOnly")}</p>
    </RadioGroup>
  );
}
