...
export function ServiceSelectionModal({ currentId, services, onClose, onSelect }: Readonly<{
  currentId: string;
  ...
  return (
    <Modal isOpen onOpenChange={(open) => { if (!open) onClose(); }}>
      <Modal.Backdrop><Modal.Container size="md" placement="center"><Modal.Dialog className="rounded-xl border border-admin-border bg-admin-surface">
        ...
        <Modal.Footer className="border-t border-admin-border px-5 py-4"><Button variant="outline" className="rounded-lg border-admin-border" onPress={onClose}>{t("selection.close")}</Button></Modal.Footer>
