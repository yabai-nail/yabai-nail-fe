...
export function PaymentSummaryPanel({ invoice, totals, onChange, onConfirm, onPreview }: Readonly<{
  ...
  return <Card className="h-fit gap-0 rounded-lg border-admin-border bg-admin-surface p-0 shadow-none lg:col-span-2 xl:col-span-1"><Card.Header className="flex flex-row items-center justify-between border-b border-admin-border px-4 py-3"><h2 className="font-bold text-admin-ink">Tổng thanh toán</h2><Chip size="sm" variant="soft" color={isPaid ? "success" : "warning"}><Chip.Label>{isPaid ? "Đã thanh toán" : "Chờ thanh toán"}</Chip.Label></Chip></Card.Header><Card.Content className="space-y-5 px-4 py-4">
    <dl className="space-y-3 text-sm"><SummaryRow label="Tổng tiền dịch vụ" value={formatMoney(totals.subtotal)} /><div><label htmlFor="payment-discount" className="mb-2 block text-xs font-semibold text-admin-ink">Giảm giá (¥)</label><div className="flex gap-2"><input id="payment-discount" className={fieldClassName} type="number" min="0" max={totals.subtotal} step="1" value={discount} disabled={isPaid} aria-invalid={Boolean(error)} aria-describedby={error ? "payment-discount-error" : undefined} onChange={(event) => setDiscount(event.target.value)} /><Button isIconOnly variant="outline" className="shrink-0 rounded-lg border-admin-border" isDisabled={isPaid} aria-label="Áp dụng giảm giá" onPress={applyDiscount}><PencilSquareIcon className="size-4" /></Button></div>{error ? <p id="payment-discount-error" role="alert" className="mt-1 text-xs text-danger">{error}</p> : null}</div><SummaryRow label="Khách cần thanh toán" value={formatMoney(totals.grandTotal)} strong /></dl>
