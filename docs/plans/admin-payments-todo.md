...
- Hoàn thành và kiểm chứng ngày 2026-08-16.
- Tất cả task `AP-01` đến `AP-11` đã đạt acceptance tương ứng.

## Phase 1 — Domain foundation

- [x] `AP-01` Viết payment domain tests
...
- [x] `AP-02` Implement contracts, fixtures và pure state helpers
  - Depends on: `AP-01`.
...
- [x] Tests bao phủ `0/60/100%`, số tiền lẻ, discount `0/subtotal/out-of-range` và duplicate service.
- [x] Pure helpers không dùng React, browser API, floating currency hoặc fixture page khác.

## Phase 2 — Route and context

- [x] `AP-03` Tạo route shell và bật navigation
  - Depends on: `AP-02`.
...
- [x] `AP-04` Dựng customer/appointment context
  - Depends on: `AP-03`.
...
- [x] Route/sidebar/context render đúng và không action inert.
- [x] Context transition không mutate fixture và không làm lệch invoice totals.

## Phase 3 — Service composition

- [x] `AP-05` Dựng primary service comparison và selection
...
- [x] `AP-06` Dựng additional/custom line item flow
  - Depends on: `AP-05`.
...
- [x] Dịch vụ chính và line items dùng một invoice source.
- [x] Mọi mutation hợp lệ cập nhật subtotal; invalid mutation không thay state.

## Phase 4 — Totals and confirmation

- [x] `AP-07` Dựng totals, discount và revenue split
...
- [x] `AP-08` Dựng payment method và confirmation flow
  - Depends on: `AP-07`.
...
- [x] Draft → paid hoạt động và reload khôi phục fixture.
- [x] Không network payment request, credential input hoặc sensitive log.
- [x] Paid invoice không thể đổi service/item/discount/split bằng UI hoặc helper.

## Phase 5 — Invoice and integration

- [x] `AP-09` Dựng order note và invoice preview/print
  - Depends on: `AP-08`.
...
- [x] `AP-10` Nối dashboard entry points và polish responsive
...
- [x] `AP-11` Chạy full verification và review
...
- [x] Verification record có số test, route và viewport thực tế.
